...
function OrgContextPage() {
  ...
  const fetchOrgs = useCallback(async () => {
    try {
      ...
      if ((data.orgs || []).length === 0 && data.error) {
        message.warning(data.error);
      }
    } catch (e) {
      console.warn("[kingdee-admin] 加载组织列表失败:", e);
      message.warning("获取组织列表失败，请检查企业版连接配置");
    }
  }, []);
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      }, "默认组织按 Agent + 渠道 + 用户保存。对话中不主动切换组织时，金蝶企业版工具会一直使用这里设置的组织。"),
...
function init() {
  ...
  if (QwenPaw.registerRoutes) {
    ...
    console.log("[kingdee-admin] 路由注册完成: permissions, org-context, audit-log, kd-config, kd-mapping");
  }
}
