...
2. **防跳步原则（硬性防护）**：代码追踪预览状态，未预览过则拒绝 execute=True：
   - 模型直接传 `execute=True` 跳过确认 → 会被代码阻止并返回错误
...
4. **空结果声明原则（硬性防护）**：查询返回空结果时，代码强制附加防幻觉声明：
   - 硬编码提示"系统中未找到匹配数据，请核实查询条件，不要编造替代答案"
   - 防止模型对空结果编造后续回答
...
10. **歧义追问原则**：用户指令模糊时（如"帮我处理采购订单""审核一下那个单子"），必须追问具体的单据编号和操作类型，不得自行决定或猜测
